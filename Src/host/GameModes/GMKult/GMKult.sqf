...
class(GMKult) extends(GMBase) //GMBase - базовый режим, от которого унаследованы все возможные режимы.
	
    // -----------------------------------------------------------------//
    // ------------------ Базовые настройки режима ---------------------//
    // -----------------------------------------------------------------//

    // Текстовые данные режима GMKult
	var(name,"Имя режима");
	var(desc,"Описание");
	var(descExtended,"Описание в конце игры.");

	// Длительность игры в режиме GMKult.
	var(duration,t_asMin(30)); // 30 минут
    
    // Имя карты, которая будет создана в режиме GMKult
	getterconst_func(getMapName,"Kult");
	...
	// Список ролей (имён классов) для режима GMKult, доступных до старта раунда.
	func(getLobbyRoles)
	{
		[
	...
	func(conditionToStart)
	{
        ...
        /*
            Как правило здесь описываются условия типа: 
                если на "GMKult_BasicRole1" встало 2 игрока и на "GMKult_BasicRole2" встал 1 игрок,
            ...
            callSelfParams(getCandidatesCount,"GMKult_BasicRole1") >= 2 
            && callSelfParams(getCandidatesCount,"GMKult_BasicRole2") >= 1
	...
	func(onFailReasonToStart)
	{
		...
		"Не установлено свойство onFailReasonToStart() для режима GMKult"
	};
	...
	// Вызывается когда режим GMKult выбран (до старта раунда с загруженной картой)
	func(preSetup)
	{
		objParams();
	...
	// Событие при завершении раунда в режиме GMKult
	func(onFinish)
	{
		objParams();
