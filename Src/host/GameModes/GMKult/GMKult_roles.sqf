...
// Базовая роль для режима GMKult. Рекомендуется именовать все дочерние роли для этого режима с постфиксом имени режима и префиксом R (Role)
// Пример: RCaveExplorer_GMKult
class(GMKult_BasicRole) extends(BasicRole) // BasicRole - базовая роль, от которой унаследованы все возможные роли.
	...
	// Установите true требуется для тех ролей, без которых режим GMKult не сможет полноценно функционировать.
...
/*
	Определяем роли, унаследованные от GMKult_BasicRole

class(RDebugRole_GMKult) extends(GMKult_BasicRole)
	//change if need
	var(name,"Debug role");
