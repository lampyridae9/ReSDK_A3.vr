...
// Базовая роль для режима GMBunker. Рекомендуется именовать все дочерние роли для этого режима с постфиксом имени режима и префиксом R (Role)
// Пример: RCaveExplorer_GMBunker
class(GMBunker_BasicRole) extends(BasicRole) // BasicRole - базовая роль, от которой унаследованы все возможные роли.
	...
	// Установите true требуется для тех ролей, без которых режим GMBunker не сможет полноценно функционировать.
...
/*
	Определяем роли, унаследованные от GMBunker_BasicRole

class(RDebugRole_GMBunker) extends(GMBunker_BasicRole)
	//change if need
	var(name,"Debug role");
