...
class(GMBunker) extends(GMBase) //GMBase - базовый режим, от которого унаследованы все возможные режимы.
	
    // -----------------------------------------------------------------//
    // ------------------ Базовые настройки режима ---------------------//
    // -----------------------------------------------------------------//

    // Текстовые данные режима GMBunker
	var(name,"Имя режима");
	var(desc,"Описание");
	var(descExtended,"Описание в конце игры.");

	// Длительность игры в режиме GMBunker.
	var(duration,t_asMin(30)); // 30 минут
    
    // Имя карты, которая будет создана в режиме GMBunker
	getterconst_func(getMapName,"Bunker");
	...
	// Список ролей (имён классов) для режима GMBunker, доступных до старта раунда.
	func(getLobbyRoles)
	{
		[
	...
	func(conditionToStart)
	{
        ...
        /*
            Как правило здесь описываются условия типа: 
                если на "GMBunker_BasicRole1" встало 2 игрока и на "GMBunker_BasicRole2" встал 1 игрок,
            ...
            callSelfParams(getCandidatesCount,"GMBunker_BasicRole1") >= 2 
            && callSelfParams(getCandidatesCount,"GMBunker_BasicRole2") >= 1
	...
	func(onFailReasonToStart)
	{
		...
		"Не установлено свойство onFailReasonToStart() для режима GMBunker"
	};
	...
	// Вызывается когда режим GMBunker выбран (до старта раунда с загруженной картой)
	func(preSetup)
	{
		objParams();
	...
	// Событие при завершении раунда в режиме GMBunker
	func(onFinish)
	{
		objParams();
