...
class(GMKislo) extends(GMBase) //GMBase - базовый режим, от которого унаследованы все возможные режимы.
	
    // -----------------------------------------------------------------//
    // ------------------ Базовые настройки режима ---------------------//
    // -----------------------------------------------------------------//

    // Текстовые данные режима GMKislo
	var(name,"Имя режима");
	var(desc,"Описание");
	var(descExtended,"Описание в конце игры.");

	// Длительность игры в режиме GMKislo.
	var(duration,t_asMin(30)); // 30 минут
    
    // Имя карты, которая будет создана в режиме GMKislo
	getterconst_func(getMapName,"Kislo");
	...
	// Список ролей (имён классов) для режима GMKislo, доступных до старта раунда.
	func(getLobbyRoles)
	{
		[
	...
	func(conditionToStart)
	{
        ...
        /*
            Как правило здесь описываются условия типа: 
                если на "GMKislo_BasicRole1" встало 2 игрока и на "GMKislo_BasicRole2" встал 1 игрок,
            ...
            callSelfParams(getCandidatesCount,"GMKislo_BasicRole1") >= 2 
            && callSelfParams(getCandidatesCount,"GMKislo_BasicRole2") >= 1
	...
	func(onFailReasonToStart)
	{
		...
		"Не установлено свойство onFailReasonToStart() для режима GMKislo"
	};
	...
	// Вызывается когда режим GMKislo выбран (до старта раунда с загруженной картой)
	func(preSetup)
	{
		objParams();
	...
	// Событие при завершении раунда в режиме GMKislo
	func(onFinish)
	{
		objParams();
