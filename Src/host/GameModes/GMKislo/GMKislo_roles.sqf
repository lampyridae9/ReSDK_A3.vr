...
// Базовая роль для режима GMKislo. Рекомендуется именовать все дочерние роли для этого режима с постфиксом имени режима и префиксом R (Role)
// Пример: RCaveExplorer_GMKislo
class(GMKislo_BasicRole) extends(BasicRole) // BasicRole - базовая роль, от которой унаследованы все возможные роли.
	...
	// Установите true требуется для тех ролей, без которых режим GMKislo не сможет полноценно функционировать.
...
/*
	Определяем роли, унаследованные от GMKislo_BasicRole

class(RDebugRole_GMKislo) extends(GMKislo_BasicRole)
	//change if need
	var(name,"Debug role");
