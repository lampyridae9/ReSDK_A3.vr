...
function(ma_transportDispatch)
{
    ...
    if !(_operation in ["getCapabilities","inspectScene","inspectObjects","applyPatch","captureViews","catalogPage","probeGeometry","buildProbe","launchRuntimeProbe"]) exitWith {
    ...
    if (_operation in ["applyPatch","captureViews","catalogPage","probeGeometry","buildProbe","launchRuntimeProbe"] && {_expected isNotEqualTo ma_revision}) exitWith {
    ...
    if (_operation == "buildProbe") exitWith {[_requestId,call ma_buildProbe] call ma_transportWrapInternal};
    if (_operation == "launchRuntimeProbe") exitWith {[_requestId,call ma_launchRuntimeProbe] call ma_transportWrapInternal};
